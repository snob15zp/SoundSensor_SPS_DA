...
struct sps_counter ble_data_counter __SECTION_ZERO("retention_mem_area0");
#endif
...
bool sps_server_send_data( void *msg, uint16_t att_idx, uint8_t conidx, uint16_t data_len )
{
    ...
    req->length = data_len;
    req->handle = sps_env->shdl + att_idx;
    req->seq_num=TX_NTF_SEQUENCE_NUMBER;
...
/**
 ****************************************************************************************
 * @brief Read characteristic value from service. Function checks if attribute exists, and if so return its length and pointer to data.
