 ...
 * @addtogroup Drivers
 * @{
 * @addtogroup WKUPCT_QUADEC WKUPCT / QUADEC
 * @brief Wakeup IRQ & Quadrature Decoder driver API
 * @{
 *
...
/// Wake up pin polarity
enum
{
    /// The event counter is incremented if the respective input goes HIGH
    WKUPCT_PIN_POLARITY_HIGH = 0,

    /// The event counter is incremented if the respective input goes LOW
...
/// WKUPCT_QUADEC error code
typedef enum
{
    /// WKUPCT_QUADEC reservation error
    WKUPCT_QUADEC_ERR_RESERVED = -1,

    /// WKUPCT_QUADEC reservation no error
    WKUPCT_QUADEC_ERR_OK = 0,
} wkupct_quadec_error_t;

/// Callback type to be associated with wakeup controller.
typedef void (*wakeup_handler_function_t)(void);

/// Callback type to be associated with quadrature decoder.
typedef void (*quad_encoder_handler_function_t)(int16_t qdec_xcnt_reg, int16_t qdec_ycnt_reg, int16_t qdec_zcnt_reg);

/// Quad decoder channel X port selection
typedef enum
{
#if defined (__DA14531__)
...
#if defined (__DA14531__)
/// Quad decoder channel X counting mode
typedef enum
{
    /// Normal counting
...
/// Quad decoder channel Y port selection
typedef enum
{
#if defined (__DA14531__)
...
#if defined (__DA14531__)
/// Quad decoder channel Y counting mode
typedef enum
{
    /// Normal counting
...
/// Quad decoder channel Z port selection
typedef enum
{
#if defined (__DA14531__)
...
#if defined (__DA14531__)
/// Quad decoder channel Z event mode
typedef enum
{
    /// Normal counting
...
/// Quad decoder initialization struct
typedef struct
{
#if defined (__DA14531__)
    /// Event mode of channel X
    CHX_EVENT_MODE_t chx_event_mode;

    /// Event mode of channel Y
    CHY_EVENT_MODE_t chy_event_mode;

    /// Event mode of channel Z
    CHZ_EVENT_MODE_t chz_event_mode;
#endif

    /// Channel X port selection
    CHX_PORT_SEL_t chx_port_sel;

    /// Channel Y port selection
    CHY_PORT_SEL_t chy_port_sel;

    /// Channel Z port selection
    CHZ_PORT_SEL_t chz_port_sel;

    /// Clock divider
    uint16_t qdec_clockdiv;

    /// Number of events before interrupt
...
/**
 ****************************************************************************************
 * @brief Quadrature Decoder or Wake up interrupt handler
 ****************************************************************************************
 */
#if !defined(__EXCLUDE_WKUP_QUADEC_HANDLER__)
void WKUP_QUADEC_Handler(void);
#endif //(__EXCLUDE_WKUP_QUADEC_HANDLER__)
