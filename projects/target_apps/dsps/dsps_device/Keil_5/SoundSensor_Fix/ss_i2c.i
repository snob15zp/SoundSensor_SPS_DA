...
#line 2 "..\\src\\i2c\\ss_i2c.c"
#line 3 "..\\src\\i2c\\ss_i2c.c"
#line 4 "..\\src\\i2c\\ss_i2c.c"
...
static const i2c_cfg_t i2c_cfg = {
  .clock_cfg.ss_hcnt = (0x00000048),
  .clock_cfg.ss_lcnt = (0x0000004F),
  .clock_cfg.fs_hcnt = (0x00000008),
  .clock_cfg.fs_lcnt = (0x00000017),
...
{
  i2c_error_code err;
  uint8_t wByte = 0 << 7 | 0 << 6 |
                  0 << 5 | 0 << 4 |
                  0 << 3 | 1 << 2 |
                  1 << 1 | 0 << 0;
  err = i2c_eeprom_write_byte((0x01), wByte);               
  if(err != I2C_NO_ERROR) return err;
  err = i2c_eeprom_write_byte((0x02), 0x00);
  if(err != I2C_NO_ERROR) return err;
  wByte = 1 << 2;                                                       
  err = i2c_eeprom_write_byte((0x03), wByte);
  return err;  
}
...
ledsTime = sx_encounter;	
  ...
  {
    i2c_eeprom_read_byte((0x00), &outData);
  }
	
	
	i2c_eeprom_write_byte((0x00), 0xBF&(~0x10) ); 
...
void ss_i2c_test (void)
{
      ...
      i2c_eeprom_write_byte((0x00), i2cBuff[cnt]);
      cnt++;
      if(cnt == 6){cnt = 0;}
...
volatile struct {
  uint16_t VLTG_READ_EN       : 1;                                      
	uint16_t PWR_ENABLED			  : 1;				                              
...
static void scanInputs(void)
{
  ...
  if(i2c_eeprom_read_byte((0x00), &inpData) == I2C_NO_ERROR)  
  {
    if(inpData & 0x04)                                            
    {
      if(sw3.debounceCount < ((uint8_t)2))
      {
        if(++sw3.debounceCount == ((uint8_t)2))                              
        {
          if(sw3.isPressed == 0)
          {
            sw3.isPressed = 1;
            sw3.pressEventFixed = 1;
            sw3.pressEventTime = sx_encounter;
          }
        }
      }
    } 
    else                                                                
    {
      if(sw3.debounceCount > 0)
      {
        if(--sw3.debounceCount == 0)
        {
          ...
          {
            sw3.isPressed = 0;
            sw3.unpressEventFixed = 1;
            sw3.pressedStateTime = sx_encounter - sw3.pressEventTime;
          }
        }
      }        
    }
    ...
    if((inpData & 0x02) == 0)                                     
    {
      if(sw1.debounceCount < ((uint8_t)2))
      {
        if(++sw1.debounceCount == ((uint8_t)2))
        {
          if(!sw1.isPressed)
          {
            sw1.isPressed = 1;
            sw1.pressEventFixed = 1;
            sw1.pressEventTime = sx_encounter;
            sw1.numOfClicks++;          
          }
        }
      }    
    ...
    else
    {
      if(sw1.debounceCount > 0)
      {
        if(--sw1.debounceCount == 0)
        {
          ...
          {
            sw1.isPressed = 0;
            sw1.unpressEventFixed = 1;
            sw1.pressedStateTime = sx_encounter - sw1.pressEventTime;
          }
        }
      }        
    }
...
static btnCmd_en decodeButtonsState(void)
{
  ...
  if(sw3.isPressed && sw3.pressEventFixed)                              
  {
    if(sw1.numOfClicks == 0)                                            
    {
      if((sx_encounter - sw3.pressEventTime) >= (5000000/(1000000/8)))                          
      {
        sw3.pressEventFixed = 0;                                    
        return BTN_SW3_LONG;                                            
      }
    }
  }
  
  if(sw3.unpressEventFixed)                                             
  {
    sw3.unpressEventFixed = 0;
    result = BTN_CMD_NO;
    if(sw3.pressedStateTime < (5000000/(1000000/8)))
    {
  ...
  if(sw1.unpressEventFixed)                                             
  {
    sw1.unpressEventFixed = 0;
    if(!sw3.isPressed)
    {
      sw1.numOfClicks = 0;  
...
{
  if(Status.PWR_ENABLED)                                                
  {
    outData &= ~0x08;                                            
    outData |= 0x01;                                          
    Status.PWR_ENABLED = 0;                                             
  } 
  else
  {
    outData |= 0x08;                                             
    outData &= ~0x01;                                         
    Status.PWR_ENABLED = 1;                                             
  }
