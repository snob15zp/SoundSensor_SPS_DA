#line 1 "..\\src\\SPI_ADC\\ADC_flash.c"
...
#line 5 "..\\src\\SPI_ADC\\ADC_flash.c"
...
#line 6 "..\\src\\SPI_ADC\\ADC_flash.c"
...
#line 7 "..\\src\\SPI_ADC\\ADC_flash.c"
...
#line 8 "..\\src\\SPI_ADC\\ADC_flash.c"
...
#line 9 "..\\src\\SPI_ADC\\ADC_flash.c"
...
#line 10 "..\\src\\SPI_ADC\\ADC_flash.c"




#line 21 "..\\src\\SPI_ADC\\ADC_flash.c"
...
{
int rezult_Start; 	
    ss_spi_init(((uint32_t)GPIO_PIN_4 | ((uint32_t)GPIO_PORT_0 << 4) | ((uint32_t)GPIO_PIN_11 << 8) | ((uint32_t)GPIO_PORT_0 << 12) | ((uint32_t)GPIO_PIN_0 << 16) | ((uint32_t)GPIO_PORT_0 << 20) | ((uint32_t)GPIO_PIN_3 << 24) | ((uint32_t)GPIO_PORT_0 << 28)),&UPS_spi_cfg);		 
	...
	rezult_find_AddrNewRecord = find_AddrNewRecord((0x35000), (0x3F000), &AddrNewRecord);	
	if ( rezult_find_AddrNewRecord == 0 )
	{
 		AF_V_AddADCdataToFIFO(0x80 | F_PWR_on, callerFunction);		
...
uint8_t dummy;	
	...
	AF_V_AddADCdataToFIFO(0x82, callerFunction);
	AF_V_AddADCdataToFIFO(0x84, systick_time_tek & 0x00ffff);	
	AF_V_AddADCdataToFIFO(0x85, (systick_time_tek>>16) & 0x00ffff);	
	...
	while ( RingBuffer_get_ch8(&byteFlash) == 0 )
	{
		...
		(* ( volatile uint16_t*)((0x50001204))=(0x1C) );
		dummy = (* ( volatile uint16_t*)(&spi->SPI_FIFO_READ_REGF) ) ;    
		dummy = (* ( volatile uint16_t*)(&spi->SPI_FIFO_READ_REGF) ) ;   
		dummy = (* ( volatile uint16_t*)(&spi->SPI_FIFO_READ_REGF) ) ;  
		dummy = dummy;
		...
		(* ( volatile uint16_t*)(&spi->SPI_FIFO_WRITE_REGF)=(byteFlash) );	
	
		while (( ((* ( volatile uint16_t*)(&spi->SPI_FIFO_STATUS_REGF) )&( (uint16_t)((0x8000)) )) >> (((0x8000))&0x0001? 0:((0x8000))&0x0002? 1:((0x8000))&0x0004? 2:((0x8000))&0x0008? 3: ((0x8000))&0x0010? 4:((0x8000))&0x0020? 5:((0x8000))&0x0040? 6:((0x8000))&0x0080? 7: ((0x8000))&0x0100? 8:((0x8000))&0x0200? 9:((0x8000))&0x0400?10:((0x8000))&0x0800?11: ((0x8000))&0x1000?12:((0x8000))&0x2000?13:((0x8000))&0x4000?14: 15) ) == SPI_TRANSACTION_IS_ACTIVE)
		{
		};
	
		(* ( volatile uint16_t*)((0x50001224))=(SPI_CS_NONE) );
		(* ( volatile uint16_t*)((0x50003002))=(1 << GPIO_PIN_1) );
		delay_200us();
	}
...
int8_t spi_flash_StartProgramByteSequent(uint8_t *wr_data_ptr, uint32_t address, uint16_t size)
{
    ...
    spi_cs_low();
    spi_access((((uint32_t)0xAD) << 24) | address);
...
uint32_t V_L, V_R, V_new;	
	...
	if ( V_L >= ((uint32_t)0x00FFFFFF) )
	{
		*ptr = Addr_L;
		return 0; 	
	}
	if ( V_R < ((uint32_t)0x00FFFFFF) )
	{
		*ptr = 0;
		return (-1);		
	}		
