#line 1 "..\\..\\..\\..\\..\\sdk6\\sdk\\platform\\driver\\spi_flash\\spi_flash.c"
...
#line 16 "..\\..\\..\\..\\..\\sdk6\\sdk\\platform\\driver\\spi_flash\\spi_flash.c"
#line 17 "..\\..\\..\\..\\..\\sdk6\\sdk\\platform\\driver\\spi_flash\\spi_flash.c"
#line 18 "..\\..\\..\\..\\..\\sdk6\\sdk\\platform\\driver\\spi_flash\\spi_flash.c"
...
int8_t spi_flash_wait_till_ready(void)
{
    for (uint32_t status_read_cnt = 0; status_read_cnt < 2000000; status_read_cnt++)
    {
        if ((spi_flash_read_status_reg() & 0x01) == 0)
        {
            return (0);
...
int8_t spi_flash_auto_detect(uint8_t *dev_id)
{
    ...
    {
        {W25X10CL_DEV_INDEX,    0xEF3011,      0x20000},
        {W25X20CL_DEV_INDEX,    0xEF3012,      0x40000},
        {AT25DX011_DEV_INDEX,   0x1F4200,     0x20000},
        {MX25V1006E_DEV_INDEX,  0xC22011,    0x20000},
        {MX25R1035F_DEV_INDEX,  0xC22811,    0x20000},
        {MX25R2035F_DEV_INDEX,  0xC22812,    0x40000},
        {P25Q10U_DEV_INDEX,     0x856011,       0x20000},
        {GD25WD20_DEV_INDEX,    0xC86412,      0x40000},
        {P25Q11U_DEV_INDEX,     0x854011,       0x20000},
    };
...
static int8_t spi_flash_write_enable(uint8_t command)
{
    ...
    if ((spi_flash_read_status_reg() & 0x02) ==  0x02)
    {
        return (0);
    }
...
int8_t spi_flash_set_write_enable(void)
{
    return spi_flash_write_enable(0x06);
}

int8_t spi_flash_write_enable_volatile(void)
{
    return spi_flash_write_enable(0x50);
}
...
int8_t spi_flash_set_write_disable(void)
{
    ...
    if ((spi_flash_read_status_reg() & 0x02) == 0)
    {
        return (0);
    }
...
void spi_flash_read_config_reg(uint32_t *data)
{
    ...
    *data = (spi_transaction((uint32_t) (0x15 << 24)) >> 8) & 0x0000FFFF;
}
...
int8_t spi_flash_write_status_config_reg(uint32_t data)
{
    ...
    spi_set_bitmode(SPI_MODE_32BIT);
    spi_transaction((0x01 << 24) | (data & 0x00FFFFFF));
...
{
    spi_set_bitmode(SPI_MODE_16BIT);
    *data = spi_transaction((uint32_t) (0x2B << 8));
}
...
int8_t spi_flash_configure_memory_protection(uint8_t data)
{
    ...
    status = spi_flash_write_enable(0x06);
    if (status != (0))
    {
        return status;
    }

    status = spi_flash_write_status_reg((spi_flash_read_status_reg() & (~0x3C)) |
                                        ((data) & (0x3C)));
...
int8_t spi_flash_page_erase(uint32_t page_address)
{
    ...
    spi_set_bitmode(SPI_MODE_32BIT);
    spi_transaction(((uint32_t) (0x81) << 24) | page_address);
...
int8_t spi_flash_chip_erase_forced(void)
{
    ...
    status = spi_flash_configure_memory_protection(0);
    if (status != (0))
    {
        return status;
    }
...
#line 716 "..\\..\\..\\..\\..\\sdk6\\sdk\\platform\\driver\\spi_flash\\spi_flash.c"
                            ...
                            uint32_t size, uint32_t *actual_size)
{
    ...
    uint32_t currentEndOfPage = (currentAddress / 256 + 1) * 256 - 1;
...
#line 856 "..\\..\\..\\..\\..\\sdk6\\sdk\\platform\\driver\\spi_flash\\spi_flash.c"
...
#line 1101 "..\\..\\..\\..\\..\\sdk6\\sdk\\platform\\driver\\spi_flash\\spi_flash.c"
...
{
    int8_t status;
    uint32_t read_data[256/4];
    uint32_t bytes_read;

    status = spi_flash_read_data((uint8_t *) read_data, page_address, 256, &bytes_read);
    if (status != (0))
    {
        return status;
    }
    ...
    for (uint16_t data_cnt = 0; data_cnt < 256/4; data_cnt++)
    {
...
int8_t spi_flash_is_sector_empty(uint32_t sector_address)
{
    ...
    for (uint16_t i = 0; i < (4096 / 256); i++)
    {
        status = spi_flash_is_page_empty((uint32_t)((sector_address & 0xFFFF00) + (i * 256)));
        if (status != (0))
        {
            return status;
        }
...
int8_t spi_flash_is_empty(void)
{
    ...
    uint16_t sector_num = (uint16_t) (spi_flash_cfg_env.chip_size / 4096);

    for (uint16_t i = 0; i < sector_num; i++)
    {
        status = spi_flash_is_sector_empty((uint32_t)(i * 4096));
        if (status != (0))
        {
            return status;
        }
