#line 1 "..\\src\\user_config_storage.c"
...
#line 28 "..\\src\\user_config_storage.c"
#line 29 "..\\src\\user_config_storage.c"
#line 30 "..\\src\\user_config_storage.c"
#line 1 "..\\src\\version.h"
#line 31 "..\\src\\user_config_storage.c"
...
#line 32 "..\\src\\user_config_storage.c"



user_config_struct_header_t user_config_struct_header __attribute__((section("retention_mem_area0"), zero_init));



uint8_t current_config_struct_index __attribute__((section("retention_mem_area0"), zero_init));
uint8_t current_config_struct_id __attribute__((section("retention_mem_area0"), zero_init));
uint32_t prod_head_config_offset_1 __attribute__((section("retention_mem_area0"), zero_init));
uint32_t prod_head_config_offset_2 __attribute__((section("retention_mem_area0"), zero_init));
...
enum user_config_storage_error_codes user_conf_storage_init(user_config_elem_t *conf_table, uint8_t conf_table_num_of_elmts, char *version, uint8_t *version_length)
{
    ...
    uint8_t index = 0x3;
    uint32_t gpio_map = ((uint32_t)GPIO_PIN_4 | ((uint32_t)GPIO_PORT_0 << 4) | ((uint32_t)GPIO_PIN_1 << 8) | ((uint32_t)GPIO_PORT_0 << 12) | ((uint32_t)GPIO_PIN_0 << 16) | ((uint32_t)GPIO_PORT_0 << 20) | ((uint32_t)GPIO_PIN_3 << 24) | ((uint32_t)GPIO_PORT_0 << 28));
    ...
    ret = user_config_storage_read_ext_mem( (uint8_t*)&product_header, (uint32_t)0x38000, (uint32_t)sizeof(config_product_header_t) );
    if(ret < 0) {
    ...
    if (product_header.config_offset1 == 0xffffffff) {
        product_header.config_offset1 = 0x20000;
        ret = 1;
    }
    
    if (product_header.config_offset2 == 0xffffffff) {
        product_header.config_offset2 = 0x21000;
        ret = 1;
    }
    
    if(ret) {
        ret = user_flash_write_data((uint8_t*)&product_header, 0x38000, (uint32_t)sizeof(config_product_header_t));
        if(ret < 0) {
            return USER_CONFIG_STORAGE_PRODUCT_HEADER_FAILURE;
        }
    }
    ...
    if (user_config_struct_header.signature[0] != 0x70 || user_config_struct_header.signature[1] != 0x54 || user_config_struct_header.valid != 0xAA) {
        index &= ~1;
    }
    if (conf_header.signature[0] != 0x70 || conf_header.signature[1] != 0x54 || conf_header.valid != 0xAA) {
        index &= ~2;
    }
                                                                            ...
                                                                            uint32_t config_offset, uint32_t size)
{
    ...
    header->signature[1] = 0x54;
    header->valid = 0xFF;
    header->struct_id = current_config_struct_id + 1;
    memcpy(header->version, "0.1.2-B", sizeof("0.1.2-B"));
