...
//    while (uart_data_rdy_getf() && uart_sps_env.rx.size)
    while (uart_data_ready_getf(UART1)&& uart_sps_env.rx.size)
    {
        // Read the received in the FIFO
        //readData = uart_rxdata_getf();
        readData=uart_read_rbr(UART1);
   ...
   // while (uart_txfifo_full_getf())
    while (uart_txfifo_not_full_getf(UART1))
    {
        ...
        else if (uart_sps_env.tx.flow_on_pending)
        {
            ...
            uart_thr_setf(UART1, UART_XON_BYTE);
        ...
        if (uart_sps_env.tx.size)
        {
            ...
            uart_thr_setf(UART1, *uart_sps_env.tx.bufptr);
        ...
        else
        {
            ...
            uart_txdata_intr_setf(UART1, UART_BIT_DIS);
...
void uart_sps_write(uint8_t *bufptr, uint32_t size, void (*callback) (uint8_t))
{
   // Sanity check
    ...
    uart_txdata_intr_setf(UART1, UART_BIT_EN);
}
    ...
    if(idd!=NO_INT_PEND)
    {
        switch(idd)
        {
            case UART_TIMEOUT:
                ...
                if ((uart_sps_env.errordetect == UART_ERROR_DETECT_ENABLED) && uart_fifo_error_getf(UART1))
                {
...
{
     if ((uart_tx_empty_getf(UART1) == 0) || (uart_thr_empty_getf(UART1) == 0) || !uart_sps_is_rx_fifo_empty())    
    {
        return true;
    }
    else
...
bool uart_sps_is_rx_fifo_empty(void)
{
    ...
    return (uart_data_ready_getf(UART1)==0);
}
...
uint32_t uart_sps_get_tx_buffer_size(void)
{
    uint8_t isr_enabled = uart_intr_config_getf(UART1)&ETBEI_dlh1;
