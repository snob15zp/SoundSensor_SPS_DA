...
//        #warning "LOG_HEAP_USAGE must not be set when building for production (DEVELOPMENT_DEBUG is 0)"
//    #else
...
#if (EXTERNAL_WAKEUP)
void ext_wakeup_enable(uint32_t port, uint32_t pin, uint8_t polarity, uint16_t debounce_time)
{
    ...
    wkupct_enable_irq(WKUPCT_PIN_SELECT((GPIO_PORT) port, (GPIO_PIN) pin), // Select pin
                      ...
                      1, // 1 event
                      debounce_time); // debouncing time = 0
}
...
#if defined (__DA14531__)
    // Only wakeup the BLE when XTAL32M is not used as system clock
    // in order to avoid to remain wake up forever. This happened
    // when external host woked 531 up immediately(0.1msec-4.5msec) after finishing of dlg_slp_isr()
    if((GetWord16(CLK_CTRL_REG) & RUNNING_AT_XTAL32M) == 0)
