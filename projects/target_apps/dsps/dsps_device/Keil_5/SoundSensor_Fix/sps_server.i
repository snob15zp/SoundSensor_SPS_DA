#line 1 "..\\..\\common\\src\\profiles\\spss\\sps_server.c"
...
#line 30 "..\\..\\common\\src\\profiles\\spss\\sps_server.c"
#line 31 "..\\..\\common\\src\\profiles\\spss\\sps_server.c"
#line 32 "..\\..\\common\\src\\profiles\\spss\\sps_server.c"
#line 33 "..\\..\\common\\src\\profiles\\spss\\sps_server.c"
#line 34 "..\\..\\common\\src\\profiles\\spss\\sps_server.c"
#line 35 "..\\..\\common\\src\\profiles\\spss\\sps_server.c"
...
#line 36 "..\\..\\common\\src\\profiles\\spss\\sps_server.c"
#line 37 "..\\..\\common\\src\\profiles\\spss\\sps_server.c"
#line 38 "..\\..\\common\\src\\profiles\\spss\\sps_server.c"
...
#line 39 "..\\..\\common\\src\\profiles\\spss\\sps_server.c"
...
uint8_t sps_init(struct prf_task_env *env, uint16_t *start_hdl, uint16_t app_task, uint8_t sec_lvl, struct spss_db_cfg *params)
{
    ...
    status = attm_svc_create_db_128(SPSS_IDX_SVC, start_hdl, 0,
            SPSS_IDX_NB, 0, env->task, sps_att_db,
            (sec_lvl & PERM_MASK_SVC_AUTH) | (sec_lvl & PERM_MASK_SVC_EKS) | (((PERM_RIGHT_ENABLE) << (PERM_POS_SVC_PRIMARY)) & (PERM_MASK_SVC_PRIMARY)));
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        memset(spss_env, 0, sizeof(struct spss_env_tag));
        
        env->env = (prf_env_t *)spss_env;
        spss_env->shdl = *start_hdl;
        spss_env->max_nb_att = SPSS_IDX_NB;
        spss_env->prf_env.app_task = app_task
                | ((((sec_lvl) & (PERM_MASK_SVC_MI)) >> (PERM_POS_SVC_MI)) ? (((PERM_RIGHT_ENABLE) << (PERM_POS_PRF_MI)) & (PERM_MASK_PRF_MI)) : (((PERM_RIGHT_DISABLE) << (PERM_POS_PRF_MI)) & (PERM_MASK_PRF_MI)));
        spss_env->prf_env.prf_task = env->task | (((PERM_RIGHT_DISABLE) << (PERM_POS_PRF_MI)) & (PERM_MASK_PRF_MI));
...
static void sps_destroy(struct prf_task_env *env)
{
    ...
    struct spss_env_tag *spss_env = ((struct spss_env_tag *)prf_env_get((TASK_ID_SPS_SERVER)));
...
static void sps_create(struct prf_task_env *env, uint8_t conidx)
{
    ...
    spss_env->tx_busy_flag = 0;
    spss_env->tx_flow_en = 1;
...
static void sps_cleanup(struct prf_task_env *env, uint8_t conidx, uint8_t reason)
{
    ...
    for (att_idx = 1; att_idx < SPSS_IDX_NB; att_idx++)
    {
        ...
        if (sps_att_db[att_idx].uuid_size == 0x0002 &&
...
void sps_server_indicate_tx_flow_ctrl(uint8_t flow_ctrl, uint8_t conidx)
{
    ...
    struct sps_server_tx_flow_ctrl_ind * ind = (struct sps_server_tx_flow_ctrl_ind*) ke_msg_alloc(SPS_SERVER_TX_FLOW_CTRL_IND, prf_dst_task_get(&(sps_env->prf_env) ,conidx), prf_src_task_get(&(sps_env->prf_env) ,conidx), sizeof(struct sps_server_tx_flow_ctrl_ind));
...
{
    struct prf_server_error_ind *ind = (struct prf_server_error_ind*) ke_msg_alloc(ind_msg_id, TASK_APP, prf_get_task_from_id(TASK_ID_SPS_SERVER), sizeof(struct prf_server_error_ind));
...
int sps_server_send_event( uint16_t att_idx, uint8_t conidx, uint8_t *p_data, uint16_t data_len )
{
    ...
    struct gattc_send_evt_cmd *req = (struct gattc_send_evt_cmd*)ke_msg_alloc(GATTC_SEND_EVT_CMD, ( (ke_task_id_t)(((conidx) << 8)|(TASK_GATTC)) ), prf_src_task_get(&(sps_env->prf_env),0), (sizeof(struct gattc_send_evt_cmd) + data_len));;
...
_Bool sps_server_send_data( void *msg, uint16_t att_idx, uint8_t conidx, uint16_t data_len )
{
    ...
    struct ke_msg * kmsg = ke_param2msg(msg);
    kmsg->dest_id =    ( (ke_task_id_t)(((conidx) << 8)|(TASK_GATTC)) );
