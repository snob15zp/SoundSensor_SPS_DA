...
static int uart_receive_byte(uint8_t *recv_byte)
{
    ...
    systick_start(40000, true); //40msec
...
static void write_byte(uart_t *uart_id, uint8_t data)
{
#if defined (CFG_UART_ONE_WIRE)
...
int FwDownload(void)
{
    ...
    write_byte(UART1, STX);                                // send start TX char
    ...
    {
        write_byte(UART1, NAK);                            // if it does not fit send ACK
        return -5;
    }
    else
    {
        write_byte(UART1, ACK);                            // if it fits send ACK
    }
    ...
    write_byte(UART1, crc_code);                           // send CRC byte
