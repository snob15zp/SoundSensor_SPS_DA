...
static uint8_t lans_init(struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl, struct lans_db_cfg* params)
{
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        memset(lans_env, 0, sizeof(struct lans_env_tag));
        ...
        if (LANS_IS_FEATURE_SUPPORTED(cfg_flag, LANS_POS_Q_MASK))
        {
            lans_env->posq = (struct lanp_posq* ) ke_malloc(sizeof(struct lanp_posq), KE_MEM_ATT_DB);
            memset(lans_env->posq, 0, sizeof(struct lanp_posq));
        }
        else
        {
            lans_env->posq = NULL;
...
void lans_exe_operation(void)
{
    ...
    ASSERT_ERROR(lans_env->op_data != NULL);
