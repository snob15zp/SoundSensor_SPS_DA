...
{   e_FunctionReturnState rstate;
	...
	{ int currl;
	  ...
	  case e_M_ADCStart: 	rstate=SSM_ADCStart(); 	
		                    SSS_SetUpTimeEvent(&AM_switch_time_event,(100000/D_SYSTICK_PERIOD_US));
			break;//2
...
__STATIC_INLINE void schedule_while_ble_on(void)
{
    ...
    while (ble_is_powered())
    {
        ...
        extern void ble_fc_process(void);
        ble_fc_process();
    #if !defined(BLE_FC_DISABLE_HEAP_MONITORING)
        void ble_fc_enable_heap_monitoring(void);
        ble_fc_enable_heap_monitoring();
        void ble_fc_adjust_flow_control(void);
        ble_fc_adjust_flow_control();
    #endif
#endif
