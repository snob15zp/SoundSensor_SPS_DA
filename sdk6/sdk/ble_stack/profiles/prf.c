    ...
    switch(KE_TYPE_GET(task_id))
    {
        ...
        case TASK_ID_ANCC:
            ...
            break;
				...
				#if (BLE_SPS_SERVER)
        case TASK_ID_SPS_SERVER:
            prf_cbs = spss_prf_itf_get();
            break;
        #endif // (BLE_PAS_SERVER)
        
        #if (BLE_SPS_CLIENT)
        case TASK_ID_SPS_CLIENT:
            prf_cbs = spsc_prf_itf_get();
            break;
        #endif // (BLE_PAS_SERVER)

        #if (BLE_REMOTE_CONFIG)
        case TASK_ID_REMOTE_CONFIG:
            prf_cbs = remote_config_prf_itf_get();
            break;
        #endif
...
#if (BLE_SPS_CLIENT)
    {TASK_ID_SPS_CLIENT,    (prfif_func_t)spsc_prf_itf_get},
#endif // (BLE_SPS_CLIENT)

#if (BLE_REMOTE_CONFIG)
		{TASK_ID_REMOTE_CONFIG,  (prfif_func_t)remote_config_prf_itf_get},
#endif
