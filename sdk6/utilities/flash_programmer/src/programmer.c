...
/*
    ...
    before the actual data packet for DA14585/586,
    for DA14531 data size is limited to 32k.
*/
uint8_t buffer[ALLOWED_DATA_UART];
#else
uint8_t *buffer = NULL;
#endif
...
#if defined(__DA14531__)
    check_gpio_hw_reset((GPIO_PORT) port, pin);
#endif
...
int main(void)
{
    ...
    *(uint8_t *)(BASE_MEMORY + ACTION_OFFSET) = 0;
    ...
    memset(buffer, 0, ALLOWED_DATA_UART);
#else
    memset(TARGET_MEMORY, 0, ALLOWED_DATA_SWD);
    *(uint8_t *)(BASE_MEMORY + RESULT_OFFSET) = ACTION_READY;
    ...
    while (1) {
        ...
        switch (action)
        {
            ...
            {
                reset_mode = (_reset_options)buffer[1];
                enable_hw_reset();
                result = ERR_OK;
                response_action(buffer, result, port_sel);
                break;
